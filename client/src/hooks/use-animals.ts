...
import { type InsertAnimal, type Animal } from "@shared/schema";
...
export function useAnimals(filters?: { search?: string; status?: string; sex?: string }) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const url = new URL(window.location.origin + api.animals.list.path);
...
export function useUpdateAnimal() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...data }: { id: number } & Partial<InsertAnimal>) => {
...
export function useFamilyTree(id: number) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json(); // Complex object, generic parse
    },
    enabled: !!id,
  });
}

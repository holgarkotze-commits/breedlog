...
## Master simulation access code — complete trace
...
`seedMasterSimulationIfNeeded` immediately returns unless `isMasterSimulationCode(code)` is true. For the master code only, it checks existing animals in the target workspace for `MASTER_SIMULATION_BATCH_MARKER` and then inserts the deterministic simulation dataset from `shared/breedlog-simulation.ts`: animals, parent relationships, mating groups, breeding events, health records, flock health events/treatments, and demo genetics bloodlines. This idempotency marker prevents repeated activation from accumulating duplicate simulation animals.
